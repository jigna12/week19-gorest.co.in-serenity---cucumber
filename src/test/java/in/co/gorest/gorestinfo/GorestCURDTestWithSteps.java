...
public class GorestCURDTestWithSteps extends TestBase {
    static String name = "Samir Shukla" + TestUtils.getRandomValue();
    static String gender = "male";
    static String email = "samir_shukla" + TestUtils.getRandomValue() + "@schiller-wuckert.biz";
    ...
    public void test006() {
        ValidatableResponse response = usersSteps.getAllUsers();
        List<?> totalRecord = response.log().all().extract().path("findAll{it.id==2351}.name");
    ...
    public void test007() {
        ValidatableResponse response = usersSteps.getAllUsers();
        List<?> totalRecord = response.log().all().extract().path("findAll{it.id==2351}.email");
    ...
    public void test009() {
        ValidatableResponse response = usersSteps.getAllUsers();
        List<String> femaleGender = response.log().all().extract().path("findAll{it.id==2351}.gender");
        Assert.assertEquals("male", femaleGender.get(0));
    }
    @Title("6.id 3819 has gender =female")
    @Test
    public void test010() {
        ValidatableResponse response = usersSteps.getAllUsers();
        List<String> femaleGender = response.log().all().extract().path("findAll{it.id==2334}.gender");
